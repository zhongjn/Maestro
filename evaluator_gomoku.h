...
namespace Maestro {
	...
	public:
		Evaluation<Gomoku> evaluate(const Gomoku& game) {
			Player cur_player = game.get_player();
			...
			v = cur_player != Player::A ? b : w;
			v = v / 5;

			for (Move<Gomoku>& m : moves) {
				Gomoku g = Gomoku(game).move(m);
				judge(g.black, g.white, b, w);
				max = cur_player == Player::A ? b : w;
				sum += max;
				p.push_back(MovePrior<Gomoku>{m, max}));
	...
	private:
        ...
        int check_dir(const HalfBoard& hb, int dr, int dc) {
